...
//        System.out.println(IntToHexStringLimit2(arr2[0]));
    ...
    public byte[] getAllByteArray() {
        int dataLength = 0;
        if (getUserdata() != null) {
            dataLength = getUserdata().length;
        }

        int allLength = WjProtocol.MIN_DATA_LEN + dataLength;
        byte[] arr = new byte[allLength];

        int index = 0;
        System.arraycopy(getHeader().getBytes(), 0, arr, index, getHeader().getBytes().length);

        index += getHeader().getBytes().length;
        System.arraycopy(getLen(), 0, arr, index, getLen().length);

        index += getLen().length;
        arr[index] = getVer();

        index += 1;
        arr[index] = getEncrypt();

        index += 1;
        System.arraycopy(getPlat(), 0, arr, index, getPlat().length);

        index += getPlat().length;
        System.arraycopy(getMaincmd(), 0, arr, index, getMaincmd().length);

        index += getMaincmd().length;
        System.arraycopy(getSubcmd(), 0, arr, index, getSubcmd().length);

        index += getSubcmd().length;
        System.arraycopy(getFormat().getBytes(), 0, arr, index, getFormat().getBytes().length);

        index += getFormat().getBytes().length;
        System.arraycopy(getBack(), 0, arr, index, getBack().length);

        if (getUserdata() != null) {
            index += getBack().length;
            System.arraycopy(getUserdata(), 0, arr, index, getUserdata().length);

            index += getUserdata().length;
            arr[index] = getCheckSum();
        } else {
            index += 1;
            arr[index] = getCheckSum();
        }
